"""
Part 3: LSTM trained on price/volume features + FinBERT sentiment columns
(model B). Same architecture and hyperparameters as part 2; only the
input dimensionality grows from 8 to 11.

The pair (part2, part3) is the apples-to-apples ablation suggested by the
instructor: if the LSTM with sentiment beats the LSTM without sentiment on
TSLA and DOGE-USD but not on AAPL, we have evidence that Musk's tweets
carry exogenous signal about volatility on his target assets.
"""
import os
...
def run_for_ticker(ticker: str, text_daily: pd.DataFrame,
    ...
    logger.info(f"=== Part 3 ({MODEL_TAG}) - {ticker} ===")
