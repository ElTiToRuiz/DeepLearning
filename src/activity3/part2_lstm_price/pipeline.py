"""
Part 2: LSTM trained on price/volume features only (model A).

Acts as the baseline that part 3 (price + sentiment) is compared against
in the final ablation. Same architecture, same hyperparameters; only the
input dimensionality differs (8 vs 11).
"""
import os
...
TEXT_FEATURE_SET = "none"
MODEL_TAG = "LSTM_price"
...
def run_for_ticker(ticker: str, text_daily: pd.DataFrame,
    ...
    logger.info(f"=== Part 2 ({MODEL_TAG}) - {ticker} ===")
